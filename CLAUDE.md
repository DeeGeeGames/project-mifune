...
- `src/plugins/` — feature plugins:
  ...
  - `homeBase.ts` — placeholder home base screen: toggles the old summary overlay on `onScreenEnter('homeBase')`/`onScreenExit('homeBase')` + menu input/render system gated to `homeBase`. Deploy starts Wave Survival, Market opens the shop, and Loadout returns to loadout selection.
...
- Round structure lives in screen state, not a resource: the `playing` screen's state owns `waveNumber`, `phaseTimer`, `spawnTimer`, per-wave `kills` / `resourcesCollected`. `homeBase` screen carries `nextWaveNumber`, the optional last mission result, and `selectedIndex` for the menu. Only `playerState.resources` is global and carries across waves — sim entities (carrier, allied ships, enemies, projectiles, missiles, pickups, vfx, blasts, turret children) are spawned with `{ scope: 'playing' }` so ECSpresso removes them automatically on `playing` exit; re-spawned fresh on the next `onScreenEnter('playing')`.
