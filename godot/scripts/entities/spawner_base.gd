extends Area2D
class_name Spawner
# Enemy-producing region. Owns wave-scaled params, lifetime, the collision
# shape sizing, and the spawn cadence timer. HP is delegated to a
# HealthComponent child node. Per-type behavior (burst momentum, visual color)
# lives on the SpawnerConfig resource.
...
func _ready() -> void:
	...
	var max_hp: int = config.base_hp + _wave_number * config.hp_scaling
