...
- `src/campaign.ts` — static campaign node graph, node-status helpers, Wave Survival launch derivation, and persistent `campaignState` helpers.
...
- `src/plugins/` — feature plugins:
  ...
  - `waves.ts` — within-`playing`-screen spawn loop; on timer expiry triggers `setScreen('homeBase')` with the latest mission result. Also owns the `enemy:killed` / `pickup:collected` listeners that accumulate per-wave stats into the playing screen state.
...
- Ship turrets are separate entities parented to their owning ship via ecspresso's entity hierarchy (`ecs.spawnChild(shipId, …)`); resolve the owner with `ecs.getParent(turretId)`. `turret.mount` holds the three.js `Group` child so the aim system can rotate it in ship-local space — the visual mounting is independent of the ECS hierarchy. Turret entities deliberately have no `localTransform3D`, since transform3D would otherwise compose worldspace against the parent.
...
- Combat-run structure lives in screen state: the `playing` screen's state owns `missionNodeId`, `waveNumber`, `phaseTimer`, `spawnTimer`, per-wave `kills` / `resourcesCollected`. Campaign progression lives in the `campaignState` resource: current map node, selected node, completed nodes, next Wave Survival number, and the latest mission result. `homeBase` and `sectorMap` screen states are UI-local selection state. Sim entities (carrier, allied ships, enemies, projectiles, missiles, pickups, vfx, blasts, turret children) are spawned with `{ scope: 'playing' }` so ECSpresso removes them automatically on `playing` exit; re-spawned fresh on the next `onScreenEnter('playing')`.
- All simulation systems are gated with `.inScreens(['playing'])` so they pause during overlay screens. Always-on systems: renderer, camera, input, behavior-tree (plugin-internal), cursor. Screen-overlay visibility is event-driven via `world.onScreenEnter(name, …)` / `world.onScreenExit(name, …)` hooks per plugin, not polled in the render loop.
...
**Home base screen:** D-pad / arrow keys = `menuUp`/`menuDown` navigate · A button / Enter / Space = `menuConfirm` activate.
