...
- `src/plugins/` — feature plugins:
  ...
  - `waveSummary.ts` — between-wave screen: toggles `summaryEl` visibility on `onScreenEnter('waveSummary')`/`onScreenExit('waveSummary')` + menu input/render system gated to `waveSummary`. Selecting `Continue` triggers `setScreen('playing', { waveNumber: n + 1 })`.
...
- Round structure lives in screen state, not a resource: the `playing` screen's state owns `waveNumber`, `phaseTimer`, `spawnTimer`, per-wave `kills` / `resourcesCollected`. `waveSummary` screen carries `waveNumber` / `kills` / `resourcesCollected` forward for display plus a `selectedIndex` for the menu. Only `playerState.resources` is global and carries across waves — sim entities (carrier, allied ships, enemies, projectiles, missiles, pickups, vfx, blasts, turret children) are spawned with `{ scope: 'playing' }` so ECSpresso removes them automatically on `playing` exit; re-spawned fresh on the next `onScreenEnter('playing')`.
